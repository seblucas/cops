...
class Book extends Base
{
	...
	public function __construct($line)
	{
		...
		if ($this->hasCover) {
			// -DC- If cover field is set, init cover file name from calibre db
	...
	public function getUpdatedEpub($idData)
	{
		...
		try {
			...
			$epub->Cover2($this->coverFileName);
	...
	{
		return new EntryBook($this->getTitle(), $this->getEntryId(), $this->getComment(), "text/html", $this->getLinkArray(), $this);
	}
	...
	public static function getCount()
	{
		...
		$result = array();
		$entry = new Entry(localize("allbooks.title"), self::ALL_BOOKS_ID, str_format(localize("allbooks.alphabetical", $nBooks), $nBooks), "text", array(
			new LinkNavigation("?page=" . parent::PAGE_ALL_BOOKS)
		), "", $nBooks);
		array_push($result, $entry);
		if ($config['cops_recentbooks_limit'] > 0) {
			$entry = new Entry(localize("recent.title"), self::ALL_RECENT_BOOKS_ID, str_format(localize("recent.list"), $config['cops_recentbooks_limit']), "text", array(
	...
	{
		return self::getEntryArray(self::SQL_BOOKS_BY_AUTHOR, array(
			$authorId
		), $n);
	}
	...
	{
		return self::getEntryArray(self::SQL_BOOKS_BY_RATING, array(
			$ratingId
		), $n);
	}
	...
	{
		return self::getEntryArray(self::SQL_BOOKS_BY_PUBLISHER, array(
			$publisherId
		), $n);
	}
	...
	{
		return self::getEntryArray(self::SQL_BOOKS_BY_SERIE, array(
			$serieId
		), $n);
	}
	...
	{
		return self::getEntryArray(self::SQL_BOOKS_BY_TAG, array(
			$tagId
		), $n);
	}
	...
	{
		return self::getEntryArray(self::SQL_BOOKS_BY_LANGUAGE, array(
			$languageId
		), $n);
	}
	...
	public static function getBooksByCustom($customId, $id, $n)
	{
		...
		return self::getEntryArray($query, array(
			$id
		), $n);
	}
...
order by substr (upper (sort), 1, 1)", "substr (upper (sort), 1, 1) as title, count(*) as count", self::getFilterString(), array(), -1);
		$entryArray = array();
		while ($post = $result->fetchObject()) {
			array_push($entryArray, new Entry($post->title, Book::getEntryIdByLetter($post->title), str_format(localize("bookword", $post->count), $post->count), "text", array(
	...
	{
		return self::getEntryArray(self::SQL_BOOKS_BY_FIRST_LETTER, array(
			$letter . "%"
		), $n, $database, $numberPerPage);
	}
